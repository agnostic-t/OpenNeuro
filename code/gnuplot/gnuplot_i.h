...
/**
  ...
  and added on 12 October 2004 by Robert Bradley
  (robert.bradley@merton.ox.ac.uk). OS X and limited Windows support added
  11-13th September 2005 - on Windows, pgnuplot and wgnuplot.exe must be in the
  current directory.
 ...
 */
/*--------------------------------------------------------------------------*/
void gnuplot_plot_xy(gnuplot_ctrl *handle, double *x, double *y, int n,
                     char *title);
...
*/
int gnuplot_splot(gnuplot_ctrl *handle, double *x, double *y, double *z, int n,
                  char *title);
...
/**
  ...
  Plots a 3d graph from a grid of points, passed in the form of a 2D array
  [x,y].
 */
/*--------------------------------------------------------------------------*/

int gnuplot_splot_grid(gnuplot_ctrl *handle, double *points, int rows, int cols,
                       char *title);
...
/**
  ...
  Plots a contour plot from a list of points, passed in the form of three arrays
  x, y and z.
...
int gnuplot_contour_plot(gnuplot_ctrl *handle, double *x, double *y, double *z,
                         int nx, int ny, char *title);
...
/**
  ...
  @param	n			Number of doubles in x (y and z must be
the the same).
  @param	title		Title of the plot.
  @return	void
...
void getPoint(void *object,gnuplot_point *point,int index,int pointCount);
  @param	obj			Pointer to an arbitrary object
  @param	point		Pointer to the returned point struct (double
x,y,z)
...
int gnuplot_splot_obj(gnuplot_ctrl *handle, void *obj,
...
int gnuplot_plot_obj_xy(gnuplot_ctrl *handle, void *obj,
                        void (*getPoint)(void *, gnuplot_point *, int, int),
                        int n, char *title);
